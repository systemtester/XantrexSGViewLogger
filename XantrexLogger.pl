...
if ($DCAmps && $DCAmps > 0) #Don't Log if Inverter is not getting the juice
{
	...
	# The Inverter does not store or calculate this
	my ($num, $mantissa);
	if ($ACPwr && $ACPwr > 0)
	{
	...
	$serial_port->write("KWHTODAY?\r"); # Inverters energy production today (KWh)
	(my $ACKWhCharCount, $ACKWh) = $serial_port->read(255);

	$ACWh = 1000 * $ACKWh;
	#$ACWh = chop($ACWh); #Convert to Wh and drop the decimal
	...
	foreach my $data (@data) #Remove trailing CR
	{
		$data =~ s/^\s+//;
		$data =~ s/\s+$//;
	}
